...
2.  Note that for the startValues() function to work effectively, the appearance zoom in the global options of R and your computer display zoom should be equal i.e.both should be at 100%.
:::
:::
...
model1 <- bolides(x=x,y=x, start = start,model = "trapezium",
...
1.  Fit the boundary line model for Maize yield and any soil property of your choice using the `bolides()` function assigning it mod_1
:::
:::
...
1.  Fit the boundary line model for Maize yield and any soil property of your choice using the `blbin()` function assigning it mod_2.
:::
:::
...
::: {#my-collapse .collapse .mt-3}
1.  Use the `maize` dataset to fit the boundary line model for maize yield and any soil property of your choice using the `blqr()` function assigning it mod_3. Use `tau` = 0.99 and `tau` = 0.95, see the implication.
...
This method fits the boundary line model based the principle of Milne et al. (2006). The blqr() function implements this approach. You can view the required arguments by accessing the function’s help page.
...
::: {#my-collapse .collapse .mt-3}
1.  Determine the boundary line for soil pH from the `Soil` dataset (within the `BLA`) and assign it mod_5. Use the `sigh = 0.4`. There is no need to test for presence of boundary as it has already been established earlier using Soil P.
:::
...
```{=html}
<script> function checkKey() { const input = document.getElementById('keyInput').value.trim().toLowerCase(); const answer = document.getElementById('answerContent'); if (input === 'model') { answer.classList.add('show'); } else { alert('Incorrect key. Please try again.'); } } function hideAnswer() { const answer = document.getElementById('answerContent'); answer.classList.remove('show'); } </script>
```
:::
...
::: {#my-collapse .collapse .mt-3}
1.  Use the results from your mod_4 for soil pH to predict the largest yields for each value of soil pH in the `soil` dataset. Replace the missing pH values with the mean and assign the predicted yields as pH.
:::
:::
...
::: {#my-collapse .collapse .mt-3}
1.  From the `maize` data, choose one variable and fit the boundary line model using a custom model of your choice.
:::
:::
